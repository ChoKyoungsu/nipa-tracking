...
// 이미지 파일 경로에서 scene 번호를 추출하는 함수
int extract_scene_number(const std::string& image_file) {
    size_t pos = image_file.rfind("scene");
    ...
    size_t end_pos = pos;
    while (end_pos < image_file.length() && isdigit(image_file[end_pos])) {
        ++end_pos;
    }

    std::string scene_num_str = image_file.substr(pos, end_pos - pos);
    if (scene_num_str.empty()) {
        return -1;
    }

    try {
        int scene_num = std::stoi(scene_num_str);
        return scene_num;
    } catch (const std::exception& e) {
        std::cerr << "Error converting scene number: " << e.what() << std::endl;
        return -1;
    }
}
...
int main() {
    ...
    for (int scene_num = start_scene; scene_num <= end_scene; ++scene_num) {
        ...
        if (!std::filesystem::exists(folder_path)) {
            std::cout << "Scene folder does not exist: " << folder_path << std::endl;
            continue;
        }
        ...
        std::cout << "Processing scene " << scene_num << " with " << image_files.size() << " images." << std::endl;
        ...
        for (const auto& image_file : image_files) {
            ...
            if (!is_video_writer_initialized) {
                ...
                video_name << "output_tracking_video_scene" << scene_num << ".mp4";
                video_writer.open(video_name.str(), cv::VideoWriter::fourcc('m', 'p', '4', 'v'), 30, cv::Size(width, height));
        ...
        // 현재 scene의 동영상 파일 저장을 마무리
        if (is_video_writer_initialized) {
            video_writer.release();
        }
        cv::destroyAllWindows();

        std::cout << "Finished processing scene " << scene_num << std::endl;
    }
